...
Ks = [1, 3, 5, 10]
CAP_LIMITS = [100, 500, 1000, 1500]  # The thresholds for Sensitivity Analysis
...
# Container for all experimental runs
all_run_results = []

for cap in CAP_LIMITS:
    print(f"Running evaluation for Cap: {cap} rows...")
    labels_order = []
    corpus_clean = []
    
    # FIXED-PREFIX SAMPLING (No shuffling)
    grouped = train_df.groupby(LABEL_COL)
    for label, group in grouped:
        mega_doc = " ".join(group.head(cap)[TEXT_COL].astype(str).tolist())
        corpus_clean.append(prep(mega_doc))
        labels_order.append(label)
    
    tokenized_docs = [doc.split() for doc in corpus_clean]
    bm25 = BM25Okapi(tokenized_docs, k1=1.5, b=0.3)
    ...
    # Store the means for this specific cap
